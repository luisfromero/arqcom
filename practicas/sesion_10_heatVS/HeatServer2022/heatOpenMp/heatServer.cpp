...
* Modificado el 23 de noviembre de 2020
*
...
// Sesión 6 de prácticas (Servidor Heat, 2ª sesión: cliente y servidor Windows)
...
int heat()
{
	...
	double mean;
	int success;
	int limi=0, lims=SIZEY;
	// Serán modificadas en posteriores versiones
...
#ifdef DIFFUSION
#define DIFFT -4*u[i][j]
#define K (1000*500*500)
#else
#define DIFFT
#define K 0.25
#endif
	...
	for (i = limi; i < lims; i++)
		for (j = 1; j < SIZEX - 1; j++)
			w[i][j] = (u[i - 1][j] + u[i + 1][j] + u[i][j - 1] + u[i][j + 1] DIFFT )* K;
	return 0;
}
